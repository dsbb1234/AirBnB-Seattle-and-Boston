...
LINK TO BLOG POST FOR PROJECT: https://david-sutula.medium.com/so-you-want-to-be-an-airbnb-host-d3967a1ad10a
...
Files in the repository:
    ...
    air_listings.csv (the cleaned listings dataset used in the notebook.  Stored on Github as a zip file)<br>
    air_calendar.csv (the cleaned calendar dataset used in the notebook.  Stored on Github as a zip file)<br>
    air_reviews.csv (the cleaned reviews dataset used in the notebook.  Stored on Github as a zip file)<br>
...
1) At 20%, there are approximately twice as many superhosts in Seattle than there are in Boston.<br>
2) Superhosts tend to be more highly rated as a group by guests<br>
3) Hosts appear to have a similar number of listings compared to superhosts<br>
4) While Superhosts tend to be grouped in similary neighborhoods in Seattle, they are not grouped in similar neighborhoods in Boston.<br>
5) Median prices per night for Hosts and Superhosts are quite similar.
